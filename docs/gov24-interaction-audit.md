...
- 2026-05-28 Playwright clicked the current Government24 `국민소통채널` footer shortcut: the closed 300x54 button opens a 1200x606 upward dialog above the footer with a title, top-right close control, and a four-column link grid. Local Playwright verified the demo now opens the same 1200x606 upward dialog shape with 21 items, four columns, and 74px link cards.
- 2026-05-28 Playwright clicked the current Government24 `정부24 안내열기` footer toggle: the footer expands to show address rows, call-center rows, extension-number guidance, and a right-side support-link list. The demo footer information toggle now mirrors that two-column contact/support layout.
- 2026-05-28 Playwright verified the demo footer satisfaction flow: `평가완료` starts disabled, selecting `만족` sets a pressed state, enables completion, and submitting exposes the status text `만족 의견이 접수되었습니다.` Hovering an unselected rating applies the expected light-blue hover state while the selected rating remains dark blue.
