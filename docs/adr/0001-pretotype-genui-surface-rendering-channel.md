...
For the pretotype, the GenUI Surface is delivered as a **Claude Artifact**: the MCP tool `compose_genui_artifact({ utterance, context })` returns a **self-contained HTML string** (the PR #1 React UI for the matching context, bundled per-context via `vite build` / `vite-plugin-singlefile`), and a host prompt instructs Claude to render that HTML **verbatim** as an HTML artifact (no paraphrase, no rebuild). Cards link out to official portals (gov.kr / hometax.go.kr / data.go.kr / law.go.kr) in a new tab; there is no in-artifact drill-in.
...
- **Three Day-0 spikes gate the whole demo** (run before any feature code): (1) Claude renders MCP-returned HTML *verbatim* as an artifact; (2) the vite-singlefile bundle fits the artifact size limit (React runtime + ~2824-line CSS → hundreds of KB; fall back to a lighter static render if exceeded); (3) the artifact sandbox permits external new-tab navigation to gov.kr.
