...
19. As a security-conscious user, I want browser assist clearly marked experimental, so that I understand its limitations.
20. As a security-conscious user, I want browser assist to require explicit approval before clicks, so that it cannot act on pages without consent.

## Implementation Decisions

- Use **TypeScript/Node.js** across the MCP server, shared schemas, domain core, browser-assist boundary, and demo UI.
- Use a **pnpm monorepo** with separate packages for schema, core domain logic, MCP server, experimental browser assist, and demo UI.
...
- Store snapshots, patches, consistency results, and cache metadata in **SQLite**.
- Support JSON fixture import/export for deterministic tests and demos.
- Implement five MVP MCP tools:
  ...
  - `getApplicationGuide`
  - `getChangeLog`
- Use official/public APIs first and read-only page extraction only as a fallback.
...
- Add an experimental Playwright-compatible browser-assist module for screen reading, step guidance, and user-approved clicks only.
...
- Test SQLite snapshot, patch, cache, and changelog behavior.
...
- Keep Playwright/browser-assist tests opt-in and separate from core CI.
...
- The MVP should describe recommendations as “candidate benefits” rather than confirmed eligibility.
- User profile input should remain non-identifying: region, age range, student/worker status, household type, and interest category.
- Browser assist must be clearly marked experimental and must require explicit user approval before navigation or clicks.
- The project should optimize for open-source adoption, clear schemas, reproducible demos, and modular contribution paths.
